...
const selectedMembershipId = ref(props.clients[0]?.membership_id || null);
const clientes = computed(() => props.clients);
const clienteSeleccionado = computed(() => {
    return props.clients.find(c => c.membership_id === selectedMembershipId.value) || props.clients[0] || null;
});
const puedeRegistrarPago = computed(() => Boolean(clienteSeleccionado.value?.payment_id));
...
const realizarPago = () => {
    ...
    if (!clienteSeleccionado.value.payment_id) return;

    form.patch(route('payments.pay', clienteSeleccionado.value.payment_id), {
        preserveScroll: true,
        onSuccess: () => {
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div class="bg-[#111111] border border-white/5 rounded-2xl p-6 shadow-xl">
                        ...
                        <div class="space-y-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                            <div v-for="c in clientes" :key="c.membership_id" 
                                @click="selectedMembershipId = c.membership_id"
                                :class="[c.membership_id === selectedMembershipId ? 'border-[#00BFA5] bg-[#00BFA5]/5' : 'border-white/5 bg-white/5']"
                    ...
                    <div class="bg-[#111111] border border-white/5 rounded-2xl p-6 shadow-xl flex flex-col">
                        ...
                        <div v-if="clienteSeleccionado && clienteSeleccionado.estado === 'Mora'" class="bg-red-950/30 border border-red-500/20 p-4 rounded-xl flex items-start gap-3 mb-6">
                            ...
                            <div>
                                ...
                                <p class="text-red-400/80 text-xs">{{ clienteSeleccionado.dias }} días de atraso.</p>
                            </div>
                        </div>
                        ...
                        <div class="bg-white/5 rounded-xl p-4 mb-4 border border-white/5">
                            ...
                            <p class="text-gray-400 text-xs">{{ clienteSeleccionado?.tel || '' }}</p>
                        </div>
                        ...
                        <div class="bg-white/5 rounded-xl p-4 mb-6 border border-white/5">
                            ...
                            <p class="text-[#00BFA5] text-4xl font-black">L {{ (clienteSeleccionado?.monto || 0) }} <span class="text-xs text-gray-500 font-normal">HNL</span></p>
                        </div>

                        <p class="text-gray-500 text-[10px] uppercase font-bold mb-3">Método de Pago</p>
                        <div class="grid grid-cols-2 gap-4 mb-8">
                            <button @click="form.metodo_pago = 'efectivo'" :class="form.metodo_pago === 'efectivo' ? 'border-[#00BFA5] text-[#00BFA5]' : 'border-white/10 text-gray-400 hover:text-white hover:border-white/30'" class="flex flex-col items-center justify-center p-4 bg-white/5 border rounded-xl hover:bg-[#00BFA5]/10 transition">
                            ...
                            <button @click="form.metodo_pago = 'tarjeta_debito'" :class="form.metodo_pago === 'tarjeta_debito' ? 'border-[#00BFA5] text-[#00BFA5]' : 'border-white/10 text-gray-400 hover:text-white hover:border-white/30'" class="flex flex-col items-center justify-center p-4 bg-white/5 border rounded-xl hover:bg-white/10 transition">
                        ...
                        <p v-if="clienteSeleccionado && !puedeRegistrarPago" class="text-xs text-yellow-500 mb-3">Este cliente está al día. No hay pagos pendientes.</p>
                        <button @click="realizarPago" :disabled="form.processing || !clienteSeleccionado || !puedeRegistrarPago" class="w-full bg-[#00BFA5] hover:bg-[#009688] text-white font-bold py-4 rounded-xl transition shadow-lg shadow-[#00BFA5]/20 mt-auto disabled:opacity-50">
                            Registrar Pago
                        </button>
                    </div>
                </div>
                ...
                <div class="bg-[#111111] border border-white/5 p-10 rounded-3xl shadow-2xl text-center max-w-lg w-full">
                    ...
                    <div class="bg-white/5 rounded-2xl p-6 mb-8 border border-white/5 space-y-4">
                        <div class="flex justify-between border-b border-white/5 pb-4">
                            ...
                            <span class="font-bold text-white">L {{ (clienteSeleccionado?.monto || 0) }} HNL</span>
                        </div>
                        <div class="flex justify-between pt-2">
                            <span class="text-gray-500">Método:</span>
                            <span class="font-bold text-white">{{ form.metodo_pago === 'efectivo' ? 'Efectivo' : 'Tarjeta' }}</span>
                        </div>
                    </div>
